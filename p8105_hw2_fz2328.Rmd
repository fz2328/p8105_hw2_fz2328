...
The dataset `pols` contains information about national politicians who are democratic or republican at any given time. It has `r nrow(pols)` rows.observations and `r ncol(pols)` columns/variables. The years in the resulting dataset range from`r min(pols$year)` to `r max(pols$year)`. Some key variables in this dataset inlcude `year` and the parties for `president`.

The dataset `snp`contains the close values of the S&P stock index on the associated date. It has `r nrow(snp)` rows.observations and `r ncol(snp)` columns/variables. The years in the resulting dataset range from`r min(snp$year)` to `r max(snp$year)`. Some key variables in this dataset inlcude `year` and `close`.

The dataset `unemployment` show the unemployment percentage in each year. It has `r nrow(unemployment)` rows.observations and `r ncol(unemployment)` columns/variables. The years in the resulting dataset range from`r min(unemployment$year)` to `r max(unemployment$year)`.

The resulting dataset `problem3_merge` has `r nrow(problem3_merge)` rows.observations and `r ncol(problem3_merge)` columns/variables. The years in the resulting dataset range from`r min(problem3_merge$year)` to `r max(problem3_merge$year)`. Some key variables in this dataset inlcude `year` `president` and `unemployment percentage`. 
